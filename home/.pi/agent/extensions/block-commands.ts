export default function (pi) {
  const dangerousPatterns = [
    { pattern: /\brm\s+(?:-[a-zA-Z]*[rR][a-zA-Z]*|--recursive)\b[^;|&\n]*?(?<=\s|["'])(?:\/|~|\$\{?HOME\}?|\.\.?|\*)\/?\*?(?=["']?(?:\s|$|[;|&]))/i, reason: "Refusing to recursively remove root, home, or similar top-level path" },
    { pattern: /\bsudo\b/i },
  ...
  pi.on("tool_call", async (event: { toolName: string; input: { command: string } }) => {
    ...
    for (const { pattern, reason } of dangerousPatterns) {
      if (pattern.test(event.input.command)) {
        return { block: true, reason: reason || "Command blocked: such things are discouraged. If you must do it, ask user to run it." };
      }
    }
  });
}
